...
class Branding:
    ...
    def has_custom_icon(self):
        return os.path.isfile(os.path.join(self.config_dir, 'logo.png'))
    ...
    def apply_build(self, rel):
        ...
        def transform(text):
            ...
            # Switch the app icon from the default SVG composition (Telegram.icon)
            # to the legacy PNG asset catalog ONLY when a custom icon (logo.png)
            # is provided next to branding.json. Otherwise the original
            # SVG-composed icon and its composer_icon_folders wiring are kept.
            if self.has_custom_icon():
                pairs.append(
                    ('app_icons = [ ":{}_icon".format(name) for name in composer_icon_folders ],', 'app_icons = [":DefaultAppIcon"],')
                )
                pairs.append(('composer_icon_folders = ["Telegram"]', 'composer_icon_folders = []'))
            for old, new in pairs:
                if old not in text:
                    return text
                text = text.replace(old, new)
